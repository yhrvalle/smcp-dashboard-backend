...
public class BlizzardApiController {
    ...
    @GetMapping(value = "mythic/{realm}/{characterName}", produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseEntity<String>> getMythicPlusProfile(@PathVariable String realm, @PathVariable String characterName) {
        return blizzardApiService.getMythicCharacterProfile(realm, characterName)
                .map(ResponseEntity::ok)
                .defaultIfEmpty(ResponseEntity.notFound().build());
    }
}
